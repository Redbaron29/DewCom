...
package com.example.datastore.database.PhoneNumberTable;
 ...
 * and allows the NumberViewModel to be a clean interface between the rest of the datastore
 * and the database.
 * <p>
...
public class NumberRepository {
    ...
    public void deleteNumber(Number number) {
        new deleteNumberAsyncTask(mNumberDao).execute(number);
    }
    ...
    /**
     *  Deletes a single number from the database.
     */
    private static class deleteNumberAsyncTask extends AsyncTask<Number, Void, Void> {
        private NumberDao mAsyncTaskNumberDao;

        deleteNumberAsyncTask(NumberDao numberDao) {
            mAsyncTaskNumberDao = numberDao;
        }

        @Override
        protected Void doInBackground(final Number... params) {
            mAsyncTaskNumberDao.deleteNumber(params[0]);
            return null;
        }
    }
